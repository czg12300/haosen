package com.heneng.heater.views.sdlv;
...
public class SlideAndDragListView<T> extends DragListView<T> implements WrapperAdapter.OnAdapterSlideListenerProxy,
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mState == STATE_DOWN || mState == STATE_LONG_CLICK) {
                    ...
                    if (mOnListItemClickListener != null && mIsWannaTriggerClick) {
                        ViewGroup v = (ViewGroup) getChildAt(position - getFirstVisiblePosition());
                        if (!isClickOnChild(v, mXDown, mYDown)) {
    ...
    public boolean isClickOnChild(ViewGroup group, int x, int y) {
        boolean result = false;
        for (int i = 0; i < group.getChildCount(); i++) {
            View v = group.getChildAt(i);
            if (v.getId() == R.id.device_item_im_power) {
                int[] location = new int[2];
                v.getLocationInWindow(location);
                Rect rect = new Rect(location[0],location[1]-v.getHeight(),location[0]+v.getWidth(),location[1]);
//                LogUtils.e("电源：" + rect.toString() + "______" + x + "----" +y+"------"+rect.contains(x,y));
                return rect.contains(x, y);
            }

            if (v instanceof ViewGroup) {
                result = isClickOnChild((ViewGroup) v, x, y);
            }
        }
